...
class Solution:
    def cloneGraph(self, node: 'Node') -> 'Node':
        ...
        if node.neighbors is None:
            return Node(1)
        return self.traverseGraph(node,visited)
        
        
    def traverseGraph(self,currentNode,visited):
        
        if currentNode in visited:
            return visited[currentNode]
        edges = []
        nodeCopy = Node(currentNode.val,edges)
        visited[currentNode] = nodeCopy
        
        for node in currentNode.neighbors:
            edges.append(self.traverseGraph(node,visited))
